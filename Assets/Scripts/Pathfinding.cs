    ...
    public void Setup(int width, int height, float cellSize)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int z = 0; z < height; z++)
            {
                ...
                Vector3 raycastOrigin = LevelGrid.Instance.GetWorldPosition(gridPosition);
                float raycastOffset = 5f;

                if (Physics.Raycast(raycastOrigin + Vector3.down * raycastOffset, Vector3.up, raycastOffset * 2f, obstaclesLayerMask))
                {
